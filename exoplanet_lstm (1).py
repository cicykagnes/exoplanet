# -*- coding: utf-8 -*-
"""exoplanet_lstm.ipynb

Automatically generated by Colaboratory.

Original file is located at
    https://colab.research.google.com/drive/1Pb1XF9Gb7dC9amjJy7jqiZPg7ieHiJ_U
"""

from google.colab import drive
drive.mount('/content/gdrive')

import tensorflow as tf
tf.test.gpu_device_name()
!nvidia-smi

import pandas as pd
import numpy as np
import matplotlib.pyplot as plt
from scipy.ndimage.filters import uniform_filter1d, gaussian_filter
from sklearn.metrics import accuracy_score, precision_score, recall_score, \
                            confusion_matrix, fbeta_score, precision_recall_curve, \
                            average_precision_score, auc
from keras import backend as K
from keras.models import Sequential, Model
from keras.layers import Dense, Dropout, Flatten,Input,  Activation
from keras.models import load_model, Model,Sequential,model_from_json
from keras.layers import LSTM, Reshape,GRU
from keras.optimizers import Adam
import warnings
warnings.filterwarnings('ignore')
from inspect import signature
import os
os.environ['TF_CPP_MIN_LOG_LEVEL'] = '2'

class model_class:

  def __init__(self):

      print("Loading datasets...")
      self.train = pd.read_csv("/content/gdrive/My Drive/exoTrain.csv", encoding= "ISO-8859-1") 
      self.test = pd.read_csv("/content/gdrive/My Drive/exoTest.csv", encoding= "ISO-8859-1") 
      self.x_train = self.train.drop('LABEL', axis=1)
      self.x_test = self.test.drop('LABEL', axis=1)
      self.y_train = self.train.LABEL
      self.y_test = self.test.LABEL
      self.x_train = np.array(self.x_train)
      self.y_train = np.array(self.y_train).reshape((-1,1))-1
      self.x_test = np.array(self.x_test)
      self.y_test = np.array(self.y_test).reshape((-1,1))-1 
    
      self.x_train = np.append(self.x_train, np.flip(self.x_train[0:37,:], axis=-1), axis=0)
      self.y_train = np.append(self.y_train, self.y_train[0:37]).reshape((-1,1)) 
      self.x_test = np.append(self.x_test, np.flip(self.x_test[0:5,:], axis=-1), axis=0)
      self.y_test = np.append(self.y_test, self.y_test[0:5]).reshape((-1,1))
        
      self.x_train = ((self.x_train - np.mean(self.x_train, axis=1).reshape(-1,1)) / np.std(self.x_train, axis=1).reshape(-1,1))
      self.x_test = ((self.x_test - np.mean(self.x_test, axis=1).reshape(-1,1)) / np.std(self.x_test, axis=1).reshape(-1,1))
      self.x_test = self.x_test.reshape(self.x_test.shape[0],self.x_test.shape[1],-1)
      print("x_test shape",self.x_test.shape)
      print("y_test",self.y_test.shape)
  
      self.model = Sequential()  
      self.model.add(LSTM(100,input_shape=(3197,1),return_sequences=True))
      self.model.add(Dropout(0.1))
      self.model.add(LSTM(100,return_sequences=True))
      self.model.add(Dropout(.1))
      self.model.add(LSTM(100,return_sequences=False))
      self.model.add(Dropout(.1))
      self.model.add(Dense(1,activation="sigmoid"))

  def shuffle_in_unison(self,a, b):
      rng_state = np.random.get_state()
      np.random.shuffle(a)
      np.random.set_state(rng_state)
      np.random.shuffle(b)
    
  def batch_generator(self,x_train, y_train, batch_size=32):
      half_batch = batch_size // 2
      x_batch = np.empty((batch_size, x_train.shape[1], 1), dtype='float32') 
      y_batch = np.empty((batch_size, y_train.shape[1]), dtype='float32') 
 
      while True:
        pos_idx = np.where(y_train[:,0] == 1)[0]
        neg_idx = np.where(y_train[:,0] == 0)[0]
            
        np.random.shuffle(pos_idx)
        np.random.shuffle(neg_idx)
 
        x_batch[:half_batch] = x_train[pos_idx[:half_batch]].reshape(half_batch,x_train.shape[1],-1)
        x_batch[half_batch:] = x_train[neg_idx[half_batch:batch_size]].reshape(half_batch,x_train.shape[1],-1)
        y_batch[:half_batch] = y_train[pos_idx[:half_batch]]
        y_batch[half_batch:] = y_train[neg_idx[half_batch:batch_size]]
        self.shuffle_in_unison(x_batch,y_batch)
        yield x_batch, y_batch
  
  def model_compile(self):

      self.model.compile(optimizer=Adam(1e-5), loss = 'binary_crossentropy', metrics=['accuracy'])
      self.hist = self.model.fit_generator(self.batch_generator(self.x_train,self.y_train, 32),validation_data=(self.x_test, self.y_test),verbose=0, epochs=5,steps_per_epoch=self.x_train.shape[0]//32)
 
      self.model.compile(optimizer=Adam(4e-5), loss = 'binary_crossentropy', metrics=['accuracy'])
      self.hist = self.model.fit_generator(self.batch_generator(self.x_train, self.y_train, 32),validation_data=(self.x_test, self.y_test),verbose=2, epochs=50,steps_per_epoch=self.x_train.shape[0]//32)
      model_json = self.model.to_json()
      with open("/content/gdrive/My Drive/LSTM_model.json", "w") as  json_file:
          json_file.write(model_json)
      self.model.save_weights("/content/gdrive/My Drive/LSTM_model.h5")
      print("Saved model to disk")

      return self.hist

  def plot_validation(self,hist):

      plt.plot(hist.history['loss'], color='b',label='loss')
      plt.plot(hist.history['val_loss'], color='r',label='validation loss')
      plt.title('Loss')
      plt.xlabel('Epochs')
      plt.legend(loc='upper right')
      plt.savefig('plot_0.png')
      plt.show()

      plt.plot(hist.history['accuracy'], color='b',label='accuracy')
      plt.plot(hist.history['val_accuracy'], color='r',label='validation accuracy')
      plt.title('Accuracy')
      plt.xlabel('Epochs')
      plt.legend(loc='upper right')
      plt.savefig('plot_1.png')
      plt.show()

  def prediction_training_data(self):
  
      print("Make predictions for training data")
      self.shuffle_in_unison(self.x_train,self.y_train)
      print(self.x_train.shape , self.y_train.shape)
      y_pred = self.model.predict(self.x_train.reshape(self.x_train.shape[0],self.x_train.shape[1],-1))[:,0]
      pred = np.empty((1,len(y_pred)), dtype=object)
      pred = np.where(y_pred>=0.5, 1, 0)
      self.y_train = np.reshape(self.y_train,len(self.y_train))
      pred = np.reshape(pred,len(pred))
    
      print("Create confusion matrix for training data")
      print('Validation for training data:')
      conf_matrix = pd.crosstab(self.y_train, pred)
      print(conf_matrix)
    
      accuracy = accuracy_score(self.y_train, pred)
      precision = precision_score(self.y_train, pred)
      recall = recall_score(self.y_train, pred)
      fbeta = fbeta_score(self.y_train, pred, 1)
      print('Accuracy: %.3f Precision: %.3f Recall: %.3f F_beta: %.3f' % (accuracy, precision, recall, fbeta))
    
      precision, recall, thresholds = precision_recall_curve(self.y_train, y_pred, pos_label=1)
      auc_pr = auc(recall, precision)
      print('Area under precision-recall-curve: %.3f' % (auc_pr))
      step_kwargs = ({'step': 'post'} if 'step' in signature(plt.fill_between).parameters else {})
      plt.step(recall, precision, color='b', alpha=0.2,where='post')
      plt.fill_between(recall, precision, alpha=0.2, color='b', **step_kwargs)
      plt.xlabel('Recall')
      plt.ylabel('Precision')
      plt.ylim([0.0, 1.05])
      plt.xlim([0.0, 1.0])
      plt.title('Precision-Recall Curve')
      plt.savefig('plot_2.png')
      plt.show()

  def prediction_test_data(self):
      self.shuffle_in_unison(self.x_test,self.y_test)
      y_pred = self.model.predict(self.x_test)[:,0] 
      pred = np.empty((1,len(y_pred)), dtype=object)
      pred = np.where(y_pred>=0.5, 1, 0)
      self.y_test = np.reshape(self.y_test,len(self.y_test))
      pred = np.reshape(pred,len(pred))
    
 
      print('Validation for test data:')
      conf_matrix = pd.crosstab(self.y_test, pred)
      print(conf_matrix)
 
      accuracy = accuracy_score(self.y_test, pred)
      precision = precision_score(self.y_test, pred)
      recall = recall_score(self.y_test, pred)
      fbeta = fbeta_score(self.y_test, pred, 1)
      print('Accuracy: %.3f Precision: %.3f Recall: %.3f F_beta: %.3f'% (accuracy, precision, recall, fbeta))
    
      precision, recall, thresholds = precision_recall_curve(self.y_test, y_pred, pos_label=1)
      auc_pr = auc(recall, precision)
      print('Area under precision-recall-curve: %.3f' % (auc_pr))
      step_kwargs = ({'step': 'post'} if 'step' in signature(plt.fill_between).parameters else {})
      plt.step(recall, precision, color='b', alpha=0.2,where='post')
      plt.fill_between(recall, precision, alpha=0.2, color='b', **step_kwargs)
      plt.xlabel('Recall')
      plt.ylabel('Precision')
      plt.ylim([0.0, 1.05])
      plt.xlim([0.0, 1.0])
      plt.title('Precision-Recall Curve')
      plt.savefig('plot_3.png')
      plt.show()

if __name__ == '__main__':
    print("In main")
    M = model_class()
    hist = M.model_compile()
    M.plot_validation(hist)
    M.prediction_training_data()
    M.prediction_test_data()

def prediction_with_trained_model():
    json_file = open('/content/gdrive/My Drive/LSTM_model.json', 'r')
    loaded_model_json = json_file.read()
    json_file.close()
    loaded_model = model_from_json(loaded_model_json)
    loaded_model.load_weights("/content/gdrive/My Drive/LSTM_model.h5") 
    print("Loaded model from disk")
    loaded_model.summary()
    return loaded_model

loaded_model = prediction_with_trained_model()